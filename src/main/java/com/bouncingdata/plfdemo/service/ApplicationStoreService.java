...
public class ApplicationStoreService {
	...
	public String getVisualization(String guid, String vGuid, String type) throws IOException {
		...
		/**
		 * TODO This if condition is not really meaningful as we have 
		 * almost done conditioning from the caller.
		 * Eliminate it or eliminate the method entirely
		 */
		if ("png".equals(type)) {
	...
	/**
	 * This method is the encapsulation for getting visualization source of
	 * both html and png, to limit the branch condition outside of this service.
	 * Used in stream page, analysis page.
	 * @param guid
	 * @param vGuid
	 * @param type
	 * @return Source (data) of the visualization
	 */
	public String getVisualizationSource(String guid, String vGuid, String type) {
		String vizSource = "";
		try {
			if("png".equals(type)) {
				vizSource = getVisualization(guid, vGuid, type);
			} else {
				if ("html".equals(type)) {
					vizSource = "visualize/app/" + guid + "/" + vGuid + "/html";
				}
			}
		}
		catch (Exception e) {
