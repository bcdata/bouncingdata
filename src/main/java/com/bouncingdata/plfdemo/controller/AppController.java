...
public class AppController {
	...
	public @ResponseBody
	AnalysisDetail getApplication(@PathVariable String guid) {
		try {
			...
			AnalysisDetail detail = new AnalysisDetail(code, datasets,
					attachments, visualsMap, dashboard);
			return detail;
		} catch (Exception e) {
	...
	public @ResponseBody
	DashboardDetail getVisualizationMap(@PathVariable String guid) {
		try {
			Analysis anls = datastoreService.getAnalysisByGuid(guid);
			if (anls == null)
				return null;
	...
	public @ResponseBody
	DashboardDetail updateDashboardForExecution(@PathVariable String guid) {
		try {
			Analysis anls = datastoreService.getAnalysisByGuid(guid);
			if (anls == null)
				return null;
	...
	public @ResponseBody
	ExecutionResult executeApp(@PathVariable String appGuid,
		...
		try {
			...
			try {
				...
				// appGuid, code,type});
				// If code has more than 256 character, an exception. Code is
				// stored at another place.No store code.Code
				String data = logmapper.writeValueAsString(new String[] { "3",
						appGuid, "", type });
				datastoreService.logUserAction(user.getId(),
						UserActionLog.ActionCode.EXECUTE_APP, data);
			} catch (Exception e) {
			...
			if (script == null)
				return new ExecutionResult(null, 0, 0, -1,
						"Application not found.");

			if (script.getUser().getId() != user.getId()) {
				return new ExecutionResult(null, 0, 0, -1,
						"No permission to run this app.");
			}

			try {
				appStoreService.saveApplicationCode(appGuid,
						script.getLanguage(), code);
			} catch (IOException e) {
				logger.error(
						"Exception occurs when trying to save appliation code",
						e);
			}

			// if analysis is public and this is the first execution
			/*
			 * if (!script.isExecuted() && script.isPublished() && script
			 * instanceof Analysis) { script.setExecuted(true);
	...
	String saveApp(
		...
		try {
			try {
				ObjectMapper logmapper = new ObjectMapper();
				String data = logmapper.writeValueAsString(new String[] { "4",
						guid, code, type, language });
				datastoreService.logUserAction(user.getId(),
						UserActionLog.ActionCode.SAVE_APP, data);
			} catch (Exception e) {
				logger.debug("Failed to log action", e);
			}
			BcDataScript script;
			...
			appStoreService.saveApplicationCode(guid, script.getLanguage(),
					code);
	...
	public @ResponseBody
	String updateDashboard(@PathVariable String guid,
		...
		try {
			...
			try {
				...
				String data = logmapper.writeValueAsString(new String[] { "3",
						guid, status, cause });
				datastoreService.logUserAction(user.getId(),
						UserActionLog.ActionCode.UPDATE_DASHBOARD, data);
			} catch (Exception e) {
				logger.debug("Failed to log action", e);
			}
			...
			/*
			 * if (app.isPublished() && "execute".equals(cause)) { // create
			 * activity datastoreService.doUpdateAction(user, app); }
			 */
	...
	public @ResponseBody
	void createDashboard(@PathVariable String guid, @PathVariable String status) {
	...
	void publishAnalysis(
			...
			@PathVariable String guid, ModelMap model, Principal principal) {
   ...
   */
	...
	public @ResponseBody
	ScraperDetail getScraper(@PathVariable String guid) {
		try {
			...
			List<Dataset> dsList = datastoreService.getScraperDataset(scr
					.getId());
