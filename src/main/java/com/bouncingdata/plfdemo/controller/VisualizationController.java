...
public class VisualizationController {
	...
	public String get(@PathVariable String guid, @PathVariable String vGuid,
			@PathVariable String type, ModelMap model) {
		try {
			String content = appStoreService
					.getVisualization(guid, vGuid, type);
			model.addAttribute("content", content);
		} catch (IOException e) {
			// log: file not found
			logger.debug(
					"Failed to find visualization file, appGuid: {}, vGuid: {}",
					guid, vGuid);
			logger.debug("", e);
		} catch (Exception e) {
			logger.debug("", e);
	...
	public String temp(@PathVariable String executionId,
			@PathVariable String name, @PathVariable String type, ModelMap model) {
		try {
			String content = appStoreService.getTemporaryVisualization(
					executionId, name, type);
			model.addAttribute("content", content);
		} catch (Exception e) {
			logger.debug("", e);
		}
	...
	public @ResponseBody
	void resizeViz(@PathVariable String guid, @PathVariable String vGuid,
			@PathVariable String type, WebRequest request, ModelMap model,
			Principal principal) {
		...
		try {
			...
			datastoreService.logUserAction(user.getId(),
					UserActionLog.ActionCode.RESIZE_VIZ, data);
		} catch (Exception e) {
		...
		try {
			...
			if (logger.isDebugEnabled()) {
				logger.debug(
						"Resize viz. {}/{} failed. Reason: Cannot parse width and height.",
						guid, vGuid);
			}
			return;
		}
