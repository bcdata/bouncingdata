...
public class EditorController {
	...
	public String openEditor(@PathVariable String guid,
			@PathVariable String mode, ModelMap model, Principal principal) {

		if (!"edit".equals(mode) && !"size".equals(mode)
		...
		try {
			...
			if (user == null
					|| !user.getUsername().equals(anls.getUser().getUsername())) {
				model.addAttribute("errorMsg",
						"You have no permission to edit this analysis!");
				return "error";
			}
			...
			try {
				...
				datastoreService.logUserAction(user.getId(),
						UserActionLog.ActionCode.OPEN_EDITOR, data);
			} catch (Exception e) {
			...
			if ("size".equals(mode)) {
				...
				if (visuals != null) {
					...
					for (Visualization v : visuals) {
						if ("html".equals(v.getType())) {
							...
							try {
								String source = appStoreService
										.getVisualization(guid, v.getGuid(),
												v.getType());
								visualsMap.put(
										v.getName(),
										new VisualizationDetail(v.getGuid(),
				...
				try {
					...
					logger.debug("Error when trying to get relation datasets",
							e);
				}
	...
	void saveDescription(
		...
		try {
			try {
				...
				datastoreService.logUserAction(user.getId(),
						UserActionLog.ActionCode.SAVE_DESCRIPTION, data);
			} catch (Exception e) {
				logger.debug("Failed to log action", e);
			}
